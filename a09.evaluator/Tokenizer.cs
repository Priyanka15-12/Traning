      ...
      public Token Next () {
         while (mN < mText.Length) {
            ...
            switch (ch) {
               ...
               case '+' or '-': // Unary if first index is a sign
                  if (mN - 1 == 0 || mEval.GetPreviousToken () is TOperator // Unary if previous token is operator or parantheses
                                  || (mEval.GetPreviousToken () is TPunctuation punct && punct.Punct == '('))
                     return new TOpUnary (mEval, ch);
                  else return new TOpArithmetic (mEval, ch);
