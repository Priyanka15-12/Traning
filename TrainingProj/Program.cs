...
namespace Training {
   ...
   internal class Double {
      ...
      public static bool IsValid (string input) {
         ...
         switch (plusCount) {
            case 2:
               if (!(input[0] == '+' && input[eIndex + 1] == '+')) return false; break;
            case 1:
               if (!(input[0] == '+' || input[eIndex + 1] == '+')) return false; break;
         }
         switch (minusCount) {
      ...
      /// <param name="input">Input string</param>
      /// <returns>Returns parsed double</returns>
      static double Exponent (string input) {
         string[] inputStringArr = input.Split ('e');
         double beforeE = inputStringArr[0].Contains ('.') ? Decimal (inputStringArr[0]) : int.Parse (inputStringArr[0]);
      ...
      public static double Parse (string input) {
         ...
         double doubleValue = input.Contains ('e') ? Exponent (input) : (input.Contains ('.') ? Decimal (input) : int.Parse (input));
         return doubleValue;
      }
      #endregion
   }
